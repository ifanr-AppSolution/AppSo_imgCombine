...
try: 
    ...
    sys.exit("Please install pillow first...")
...
def myfun():
    if len(text) == 1:
        ...
        fileout = os.path.split(str(text[0]))[0] + '/watermarked.jpg'
        saveimg(fileout, img)
        
    if len(text) == 2:
        print "\n>>> Combining and watermarking the target imgs...\n"
        img1 = Image.open(str(text[0]),'r')
        img2 = Image.open(str(text[1]),'r')
        fileout = os.path.split(str(text[0]))[0] + '/combined.jpg'
...
root = Tk()
myfiletypes = [('PNG files', '*.png'), ('All files', '*')]
open = tkFileDialog.Open(root, filetypes = myfiletypes)
Button(root, text="Open Image 1", command=openwindows).pack()
Button(root, text="Open Image 2", command=openwindows).pack()
Button(root, text="Combine/Watermark", command=myfun).pack()
root.mainloop()
